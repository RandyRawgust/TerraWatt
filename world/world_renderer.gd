...
func _render_visible_chunks(_center: Vector2i) -> void:
	for chunk_pos in WorldData.loaded_chunks.keys():
...
func _render_chunk(chunk_pos: Vector2i) -> void:
	...
	for local_y in WorldData.CHUNK_SIZE:
		for local_x in WorldData.CHUNK_SIZE:
