...
var _slot_backing: Texture2D


func _ready() -> void:
	_slot_backing = load("res://assets/ui/hotbar_slot.png") as Texture2D
	add_to_group("hotbar")
	_build_slots()
...
func _update_selection_highlight() -> void:
	for i in range(_slots.size()):
		...
		if _slot_backing:
			...
			sb.texture = _slot_backing
			sb.modulate_color = Color(1.22, 1.1, 0.98) if i == _selected_slot else Color(0.82, 0.82, 0.88)
			panel.add_theme_stylebox_override("panel", sb)
		else:
			var sb_flat: StyleBoxFlat = StyleBoxFlat.new()
			sb_flat.bg_color = Color(0.102, 0.102, 0.18)
			sb_flat.set_corner_radius_all(2)
			sb_flat.set_border_width_all(1)
			if i == _selected_slot:
				sb_flat.set_border_width_all(2)
				sb_flat.border_color = Color(0.722, 0.451, 0.2)
			else:
				sb_flat.border_color = Color(0.22, 0.22, 0.28)
			panel.add_theme_stylebox_override("panel", sb_flat)
