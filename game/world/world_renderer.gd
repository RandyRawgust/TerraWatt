...
func _build_placeholder_tileset() -> TileSet:
	# One source per WorldData tile_id 1..9 (source_id == tile_id - 1), single tile at atlas (0,0) — matches terrawatt_tileset / create_tileset.gd.
	...
	for tid in range(1, 10):
		...
		var sid: int = WorldData.tile_id_to_source_id(tid)
...
func _paint_cell(layer: int, pos: Vector2i, tile_id: int) -> void:
	if tile_id == WorldData.TILE_AIR:
		...
		return
	var source_id: int = WorldData.tile_id_to_source_id(tile_id)
	tile_map.set_cell(layer, pos, source_id, Vector2i(0, 0), 0)
