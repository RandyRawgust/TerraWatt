...
var _boiler_idle: Texture2D
var _boiler_active: Texture2D


func _ready() -> void:
	add_to_group("machines")
	_boiler_idle = load("res://assets/power/tier1/water_boiler.png") as Texture2D
	_boiler_active = load("res://assets/power/tier1/boiler_active.png") as Texture2D
	if body_sprite and _boiler_idle:
		body_sprite.texture = _boiler_idle
...
func _update_boiling(delta: float) -> void:
	...
	if current_heat >= MIN_HEAT_TO_BOIL and water_level > 0.0:
		if body_sprite and _boiler_active:
			body_sprite.texture = _boiler_active
